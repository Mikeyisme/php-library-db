...
define( 'TITLE', 'Bobbys Book Store || Your Cart' );
...
if ( !isset($_SESSION['checkout']) && $has_prod < 1 ) {
            ...
            while ( $row = $result->fetch_assoc() ) {
                ...
                if ( isset($_POST[$my_id]) ) {
                    ...
                    if ( filter_var( $my_prod, FILTER_SANITIZE_NUMBER_INT ) == $row[ "prod_id" ] ) {
                        ...
                        echo "<tr><td valign='top'>" . $row[ "prodname" ] . "</td><td valign='top'>" . $my_prod . "</td><td valign='top'>" . $row[ "prod_sku" ] . "</td><td valign='top'>" . $row[ "product_price" ] . "</td></tr>";                            
    ...
    while ( $row = $result->fetch_assoc() ) {
        ...
        if ( isset( $_SESSION[ $my_id ] ) && isset( $_POST[ 'checkout' ] ) ) {            
            ...
            if( $my_prod == "matte" ) {
                $my_count = $row[ "matte" ] - 1;
                $sql2 = "UPDATE myproducts SET matte=" . $my_count . " WHERE prod_id=" . $row[ 'prod_id' ];
            } else {
